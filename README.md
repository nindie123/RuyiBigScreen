...
教学型数据可视化大屏项目，基于 Vue 3 + TypeScript + ECharts 构建。采用暖阳金橙视觉主题。
...
大屏页面采用深色玻璃拟态（Glassmorphism）风格，暖阳金橙为主色调，共划分为六个区域：
...
- **城市访问排名（柱状图）**：主要城市访问量排行，金橙/珊瑚渐变色条
- **能力雷达模型（雷达图）**：系统在各维度的能力评估
- **实时动态与告警**：底部消息列表，展示系统事件

## 实时数据模拟

项目在 mock 模式下，通过 `src/mocks/realtimeDashboardSimulator.ts` 模拟真实业务系统的数据持续刷新：

- 顶部指标每 6 秒变化一次（访问量递增、订单波动、活跃用户微调）
- 趋势图每 18 秒追加新时间点，滑动窗口保留最近 10 个点
- 实时动态每 12 秒左右新增一条消息，保留最近 8 条
